import {FutureOrPresent} from './future-or-present';
...
describe('@FutureOrPresent decorator', () => {
    ...
    describe('can be applied to', () => {
        ...
        it('a property', () => {
            // given
            class TestClass {
                @FutureOrPresent private p: number;
            }
            // when
        ...
        it('a constructor parameter', () => {
            ...
            expect(methodInfo.parameters[0].futureOrPresent).toEqual(true);
        });

        it('a method parameter', () => {
            // given
            class TestClass {
                method(@FutureOrPresent p: number): void { /* empty */ }
            }
            // when
        ...
        it('a method return value', () => {
            // given
            class TestClass {
                @FutureOrPresent method(p: number): void { /* empty */ }
            }
            // when
            let methodInfo: MethodInfo = getMethodInfo(TestClass, 'method');
            // then
            ...
            expect(methodInfo.returnValue.futureOrPresent).toEqual(true);
        });
    ...
    describe('throws an error when applied to', () => {
        ...
        it('a static method', () => {
            ...
            expect(() => {
                class TestClass {
                    @FutureOrPresent static staticMethod(p: number): void { /* empty */ }
                }
            }).toThrowError(/Decorator @FutureOrPresent cannot be applied to static method TestClass\.staticMethod/);
        });
        ...
        it('a static method parameter', () => {
            ...
            expect(() => {
                class TestClass {
                    static staticMethod(@FutureOrPresent p: number): void { /* empty */ }
                }
            }).toThrowError(/Decorator @FutureOrPresent cannot be applied to static method TestClass\.staticMethod parameter #0/);
        });
        ...
        it('a static property', () => {
            ...
            expect(() => {
                class TestClass {
                    @FutureOrPresent private static staticP: number;
                }
            }).toThrowError(/Decorator @FutureOrPresent cannot be applied to static property TestClass\.staticP/);
        });
