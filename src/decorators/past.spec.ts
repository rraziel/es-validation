...
describe('@Past decorator', () => {
    ...
    describe('can be applied to', () => {
        ...
        it.skip('a constructor parameter', () => {
            ...
            expect(methodInfo.parameters[0].past).toEqual(true);
        });

        it('a method parameter', () => {
            // given
            class TestClass {
                method(@Past p: number): void { /* empty */ }
            }
            // when
        ...
        it('a method return value', () => {
            // given
            class TestClass {
                @Past method(p: number): void { /* empty */ }
            }
            // when
            let methodInfo: MethodInfo = getMethodInfo(TestClass, 'method');
            // then
            ...
            expect(methodInfo.returnValue.past).toEqual(true);
        });
    ...
    describe('throws an error when applied to', () => {
        ...
        it('a static method', () => {
            ...
            expect(() => {
                class TestClass {
                    @Past static staticMethod(p: number): void { /* empty */ }
                }
            }).toThrowError(/Decorator @Past cannot be applied to static method TestClass\.staticMethod/);
        });
        ...
        it('a static method parameter', () => {
            ...
            expect(() => {
                class TestClass {
                    static staticMethod(@Past p: number): void { /* empty */ }
                }
            }).toThrowError(/Decorator @Past cannot be applied to static method TestClass\.staticMethod parameter #0/);
        });
        ...
        it('a static property', () => {
            ...
            expect(() => {
                class TestClass {
                    @Past private static staticP: number;
                }
            }).toThrowError(/Decorator @Past cannot be applied to static property TestClass\.staticP/);
        });
